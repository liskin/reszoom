...
async function updateWinId(winId) {
	if (winId != chrome.windows.WINDOW_ID_NONE && winId != chrome.windows.WINDOW_ID_CURRENT) {
		await updateWin(await chrome.windows.get(winId));
	}
}

async function updateWinIdTabId(winId, tabId) {
	if (tabId != chrome.tabs.TAB_ID_NONE && winId != chrome.windows.WINDOW_ID_NONE && winId != chrome.windows.WINDOW_ID_CURRENT) {
		await updateWinTabId(await chrome.windows.get(winId), tabId);
	}
}
